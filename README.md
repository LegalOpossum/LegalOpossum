<div align="center">

# 👋 Привіт, я Дмитро  
### `LegalOpossum` | Junior Developer in progress 🚀

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=24&pause=1000&color=58A6FF&center=true&vCenter=true&width=700&lines=Починаю+шлях+у+розробці;Працюю+над+pet-проєктами;Вивчаю+JavaScript%2C+Python%2C+Git%2C+API;Будую+своє+портфоліо+на+GitHub" alt="Typing SVG" />

<br>

<img src="https://media.giphy.com/media/ZVik7pBtu9dNS/giphy.gif" width="260" alt="coding gif" />

</div>

---

## 🧠 Про мене

💻 Починаю свій шлях у розробці та поступово збираю власне портфоліо  
📚 Вивчаю **JavaScript, Python, Git, API, HTML, CSS**  
🛠️ Роблю pet-проєкти, щоб прокачувати практичні навички  
🎯 Зараз моя ціль — вирости в сильного спеціаліста та створити якісне портфоліо на GitHub  

---

## 🚀 Мій основний проєкт

<div align="center">

### [Expense Tracker](https://github.com/LegalOpossum/expense-tracker)

Сервіс для обліку витрат, роботи з транзакціями, категоріями та інтеграцією з API.

</div>

---

## 🛠️ Tech Stack

<div align="center">

<img src="https://skillicons.dev/icons?i=js,python,html,css,git,github,vscode,linux" />

</div>

---

## 📊 GitHub Stats

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=LegalOpossum&show_icons=true&theme=tokyonight&hide_border=true&border_radius=12" />
<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=LegalOpossum&layout=compact&theme=tokyonight&hide_border=true&border_radius=12" />

</div>

---

## 🔥 GitHub Streak

<div align="center">

<img src="https://streak-stats.demolab.com?user=LegalOpossum&theme=tokyonight&hide_border=true&border_radius=12" />

</div>

---

## 🧩 Зараз фокусуюсь на

- побудові реальних pet-проєктів
- роботі з API
- покращенні структури коду
- розвитку Git / GitHub workflow
- підготовці сильного портфоліо

---

## 🌌 Fun zone

<div align="center">

<img src="https://media.giphy.com/media/13HgwGsXF0aiGY/giphy.gif" width="220" alt="cat typing gif" />
<img src="https://media.giphy.com/media/LmNwrBhejkK9EFP504/giphy.gif" width="220" alt="coding gif" />
<img src="https://media.giphy.com/media/fwbZnTftCXVocKzfxR/giphy.gif" width="220" alt="tech gif" />

</div>

---

## 🐍 Contribution Snake

<div align="center">

![snake gif](https://github.com/LegalOpossum/LegalOpossum/blob/output/github-contribution-grid-snake.svg)

</div>

---

## 🌍 Connect with me

<div align="center">

<a href="https://github.com/LegalOpossum">
  <img src="https://img.shields.io/badge/GitHub-LegalOpossum-181717?style=for-the-badge&logo=github" />
</a>

</div>

---

<div align="center">

### ⭐ Дякую, що зайшов на мій профіль

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:58A6FF,100:7F5AF0&height=120&section=footer" />

</div>
